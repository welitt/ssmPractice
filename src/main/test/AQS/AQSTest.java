...
public class AQSTest {
    ...
    ExecutorService executorService = new ThreadPoolExecutor(3, 3, 10,
            TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(6), new ThreadPoolExecutor.CallerRunsPolicy());
    ...
    private void startLockyDraw() {
        ...
        for(int i=0;i<100;i++){
            executorService.execute(new Thread(() -> {
                ...
                if (present > 0) {
                    present--;
                    System.out.println(Thread.currentThread().getName() + "：我我我抢到了！！！");
                } else {
                    System.out.println("我我我啥也没有");
                }
                lock.unlock();
                try {
                    Thread.sleep(3000);
